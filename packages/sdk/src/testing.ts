...
export interface MockHarnessCore {
  ...
  system: {
    ...
    setMetrics(metrics: { cpuPercent: number; memUsedPercent: number; gpuPercent?: number; diskUsedPercent?: number; battery?: { percent: number; charging: boolean }; network?: { downloadBytesPerSecond: number; uploadBytesPerSecond: number }; extendedMetricsSampledAt?: number; extendedMetricsFresh?: boolean }): void;
...
export function createMockContext(optionsOrConfig: MockContextOptions | Record<string, unknown> = {}): {
  ...
  let systemMetrics: { cpuPercent: number; memUsedPercent: number; gpuPercent?: number; diskUsedPercent?: number; battery?: { percent: number; charging: boolean }; network?: { downloadBytesPerSecond: number; uploadBytesPerSecond: number }; extendedMetricsSampledAt?: number; extendedMetricsFresh?: boolean } = { cpuPercent: 5, memUsedPercent: 40 };
  let nextId = 0;
