...
- **Teams**: `teams/TeamsView.tsx` renders organization membership status, applied/pending revisions, sync timestamps, pending deep-link enrollment with display-name input, explicit synchronization, and leave confirmations. It presents organization-managed team pets and team plugins in dedicated lists while isolating personal content. `teams-state.ts` provides validation and error formatting.
