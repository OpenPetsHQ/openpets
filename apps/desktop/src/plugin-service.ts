...
import type { PluginAssistantCapability, PluginAssistantCapabilityExecutionOutcome, PluginAssistantCapabilityHandle } from "./plugin-sdk-assistant.js";
...
export class PluginService {
  ...
  constructor(options: PluginServiceOptions) {
    ...
    this.allowedPluginRoots = options.allowedPluginRoots ?? [join(options.userDataPath ?? "", "plugins"), join(options.userDataPath ?? "", "plugins-dev"), join(options.userDataPath ?? "", "team-plugins")];
  ...
  async setEnabled(id: string, enabled: boolean): Promise<PluginServiceResult> {
    ...
    if (record.source === "team") return this.#error("Team plugin settings are managed by your organization.");
  ...
  async saveConfig(id: string, config: unknown): Promise<PluginServiceResult> {
    const record = this.stateStore.getRecord(id);
    if (!record) return this.#error("Plugin is not installed.");
    if (record.source === "team") return this.#error("Team plugin configuration is managed by your organization.");
  ...
  executeAssistantCapability(handle: PluginAssistantCapabilityHandle, input: unknown): Promise<PluginAssistantCapabilityExecutionOutcome> {
    return this.runtime.executeAssistantCapability(handle, input);
  }
  ...
  async uninstall(id: string): Promise<PluginServiceResult> {
    ...
    const record = this.stateStore.getRecord(id);
    if (!record) return this.#error("Plugin is not installed.");
    if (record.source === "team") return this.#error("Team plugins can only be removed by leaving the organization or by organization policy.");
  ...
  async loadLocalPath(sourceFolder: string, options: { readonly autoApprove?: boolean } = {}): Promise<PluginServiceResult> {
    ...
    if (existing?.source === "team") return this.#error("A Team plugin with this id is managed by your organization.");
