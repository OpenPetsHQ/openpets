...
export async function stageTeamArtifact(
  userDataPath: string,
  item: TeamPackItem,
  bytes: Buffer,
  ...
  if (
    ...
    || bytes.byteLength > maxZipBytes
    || bytes[0] !== 0x50
    || bytes[1] !== 0x4b
    || createHash("sha256").update(bytes).digest("hex") !== item.sha256
  ) throw new Error("Team artifact is invalid or has the wrong checksum or size.");
  ...
  try {
    ...
    if (
      ...
      || result.manifest.runtime !== "javascript"
    ) throw new Error("Team plugin manifest does not match the Team Pack.");
    const checked = await readCatalogPluginManifestFromZip({
      catalogEntry: {} as never,
      zip: bytes,
    });
    if (!validatePluginConfigReplacement(checked.manifest, item.config ?? {}).ok) {
      throw new Error("Team plugin configuration does not match its manifest.");
    }
    await writeFiles(stagingPath, files);
...
export async function activateTeamArtifact(
  ...
  try {
    ...
    await fs.rm(staged.stagingPath, { recursive: true, force: true }).catch(
      () => undefined,
    );
    throw error;
  }
...
export async function removeTeamArtifact(
  ...
  await fs.rm(
    join(userDataPath, type === "pet" ? "team-pets" : "team-plugins", id),
...
function validatePetFiles(
  ...
  const petBytes = files.get("pet.json");
  const sprite = files.get("spritesheet.webp");
  if (
    !petBytes
    || !sprite
    || files.size !== 2
    || petBytes.byteLength > 128 * 1024
    || sprite.byteLength < 16
...
async function readZipFiles(bytes: Buffer): Promise<Map<string, Buffer>> {
  const zip = await new Promise<ZipFile>((resolvePromise, reject) =>
    yauzl.fromBuffer(
      bytes,
      {
        lazyEntries: true,
      ...
      (error, file) => error || !file
        ? reject(error ?? new Error("Unable to open Team artifact."))
        : resolvePromise(file),
    ));
      ...
      if (!settled) {
        ...
        reject(
          error instanceof Error
            ? error
            : new Error("Unable to read Team artifact."),
        );
      }
    };
    zip.on("error", fail);
    ...
    zip.on("entry", (entry: Entry) => {
      void readEntry(zip, entry)
        .then(({ name, content }) => {
          ...
          if (
            count > maxFiles
            || total > maxUncompressed
            || files.has(name)
          ) throw new Error("Team artifact contains too many, duplicate, or oversized files.");
  ...
  const prefixed = names.filter((name) => name.startsWith(" "));
  ...
  if (!files.has("pet.json") && !files.has("openpets.plugin.json")) {
    ...
    for (const [name, content] of files) {
      if (!name.startsWith(prefix)) {
        throw new Error("Team artifact layout is invalid.");
      }
      normalized.set(name.slice(prefix.length), content);
    }
    return normalized;
  }
  return files;
...
function readEntry(
  ...
  if (entry.fileName.endsWith("/")) {
    if (
      ...
      || normalized.includes("/../")
    ) return Promise.reject(new Error("Team artifact contains an unsafe ZIP directory."));
  ...
  ) return Promise.reject(new Error("Team artifact contains an unsafe ZIP entry."));
  return new Promise((resolvePromise, reject) =>
    zip.openReadStream(entry, (error, stream) => {
      if (error || !stream) {
        return reject(error ?? new Error("Unable to read Team artifact entry."));
      }
      const chunks: Buffer[] = [];
