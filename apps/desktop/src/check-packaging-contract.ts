...
assert.match(agentPetControllerSourceForLogging, /loadExplicitPetContent\(window, petId, display, badge, presentation\?\.getDismissToken\(\), scale\)/, "agent pet refreshes must pass the display, badge, authoritative render token, and saved pet scale explicitly.");
assert.match(agentPetControllerSourceForLogging, /onRenderNeeded: \(\) => \{[\s\S]*?const display = presentation\.getDisplay\(\);[\s\S]*?const badge = presentation\.getBadge\(\);[\s\S]*?loadExplicitPetContent\(window, petId, display, badge, presentation\.getDismissToken\(\), getPreferredPetScale\(\)\)/, "agent pet transient updates must render the authoritative display, badge, render token, and saved scale together.");
