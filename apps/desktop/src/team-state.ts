...
export type TeamEnrollmentState = {
  ...
  readonly pendingIntent?: TeamEnrollmentLink;
...
export class TeamStateStore {
  ...
  setReconciled(revision: number): TeamEnrollmentState {
    const current = this.require();
    if (!Number.isSafeInteger(revision) || revision < 0 || current.pendingRevision !== revision) {
      throw new Error("Team revision is not pending reconciliation.");
    }
    return this.commit({ ...current, reconciledRevision: revision, lastError: undefined });
  }

  setApplied(revision: number): TeamEnrollmentState {
    const current = this.require();
    if (
      !Number.isSafeInteger(revision)
      || revision < 0
      || current.reconciledRevision !== revision
      || current.pendingRevision !== revision
    ) {
      throw new Error("Team revision has not been fully reconciled.");
    }
    return this.commit({
      ...current,
...
function readState(path: string): TeamEnrollmentState | null {
  ...
  try {
    ...
    const hasValidReconciledRevision = raw.reconciledRevision === undefined
      || Number.isSafeInteger(raw.reconciledRevision) && (raw.reconciledRevision as number) >= 0;
    if (
      raw.version !== 1
      || typeof raw.installationId !== "string"
      || !/^desktop-[0-9a-f-]{36}$/.test(raw.installationId)
      || typeof raw.organizationId !== "string"
      || raw.organizationId.length > 160
      || !Number.isSafeInteger(raw.pendingRevision)
      || !Number.isSafeInteger(raw.appliedRevision)
      || (raw.pendingRevision as number) < 0
      || (raw.appliedRevision as number) < 0
      || !hasValidReconciledRevision
    ) return null;
    if (raw.desiredPack !== undefined) validateTeamPack(raw.desiredPack);
    return raw as unknown as TeamEnrollmentState;
  } catch {
    return null;
  }
}
