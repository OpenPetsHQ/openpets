...
| `openpets.system-resources` | Host CPU and RAM reporting with optional GPU and system-volume Disk metrics in a pinned HUD. | `system:metrics`, `schedule`, `storage`, `status`, `commands`, `pet.speak`, `pet.reaction`, `pet.pin`, `assets`, `config` |
