...
5. Stateful plugins (Simple Timer, Focus Buddy, Quick Reminders, System Resources) re-register their commands on each state change so the pet menu shows only what applies; every command stays in its owning plugin submenu.
